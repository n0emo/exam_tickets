...
$$LX_j(t)=0\Leftrightarrow LU_j+iLV_j=0\Leftrightarrow\begin{equation}
\left\{\begin{aligned} 
  LU_j=0\\
  LV_j=0
\end{aligned} \right.
\end{equation}$$
$U_j=Re(e^{\lambda_jt}h_j);V_j=Im(e^{\lambda_jt}h_j)\\$
$$X_j(t)=C^{(1)}_jRe(e^{\lambda_jt}h_j)
+C_j^{(2)}Im(e^{\lambda_jt}h_j)$$
--- 
### **$II \ случай: s_j=k_j>1\\$**
---
$II.1\  \lambda\in\R:mult(\lambda_j)=k_j\\$
